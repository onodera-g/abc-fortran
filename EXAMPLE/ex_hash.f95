...
program ABC303c
    ...
    call map%put(100, 1000) !キーを100、値を1000として登録
    x = map%get(1) !キーとして1を与え、対応する数値(10)を取り出す
    y = map%get(100)
    write (*, *) x, y

    !データの全削除
    call map%clear !全削除
    x = map%get(1) !キーとして1を与えるが、対応する数値はない(今回は0が返ってくる)
    y = map%get(100)
    write (*, *) x, y
    ...
    y = map%get(3)
    write (*, *) x, y
    call map%remove(2) !キーとして2を与え、対応する数値(20)を削除
    x = map%get(2)
    y = map%get(3)
    write (*, *) x, y
